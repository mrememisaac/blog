...
namespace EmemIsaac.Blog.Api
{
    public static class DatabaseUtility
    {
        ...
        public static async Task SeedDatabase(this WebApplication app)
        {
            using var scope = app.Services.CreateScope();
            var logger = scope.ServiceProvider.GetRequiredService<ILogger<Program>>();
            logger.LogInformation($"{nameof(SeedDatabase)} started");
            ...
            if (context != null)
            {
                ...
                if (result > 0)
                {
                    logger.LogInformation($"{nameof(SeedDatabase)}  completed ");
                }
                else
                {
                    logger.LogInformation($"{nameof(SeedDatabase)}  failed ");
                }
            }
        }
    }
